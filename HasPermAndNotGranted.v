(* En este archivo se demuestra la propiedad que postula que una aplicación puede contar
* con un permiso peligroso sin tenerlo individualmente otorgado *)
Require Export Exec.
...
Definition witnessState : State :=
    st (a1::a2::nil)
       (addAppValue (list idGrp) a2 nil (addAppValue (list idGrp) a1 (witnessPermGrp::nil) emptyPermGroups))
...
Qed.
    ...
    Theorem hasPermissionAndNotGrantedProof : exists 
    (s:System)
    (p:Perm)
    (a:idApp)
    (permsA : list Perm)
...
Proof.
    exists witnessSys, witnessPerm, a1, nil.
    assert (idappa12:=idAppa1a2Right).
    destruct idappa12 as [proof12 idappa12].
    assert (idappa21:=idAppa2a1Right).
    destruct idappa21 as [proof21 idappa21].
    ...
    split.
    unfold not;intros.
    unfold getDefPermsForApp in H.
    simpl in H.
    rewrite idappa12 in H.
    simpl in H.
    rewrite idappa11 in H.
    auto.
