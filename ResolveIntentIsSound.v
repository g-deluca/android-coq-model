...
Proof.
    ...
    assert (existsb (fun pair : iCmp * Intent => negb (isSomethingBool idCmp (cmpName (snd pair))) && (if idInt_eq (idI (snd pair)) (idI i) then true else false) && match map_apply iCmp_eq (running (state s)) (fst pair) with | Value c1 => existsb (fun iFil : intentFilter => actionTestBool (snd pair) iFil s && categoryTestBool (snd pair) iFil s && dataTestBool (snd pair) iFil s) (MyList.concat (map getFilters match intType (snd pair) with | intActivity => filter (fun c2 : Cmp => canStartBool c1 c2 s) (filter isActivityBool (getAllCmps a s)) | intService => filter (fun c2 : Cmp => canStartBool c1 c2 s) (filter isServiceBool (getAllCmps a s)) | intBroadcast => filter (fun c2 : Cmp => canStartBool c1 c2 s) (filter isBroadReceiverBool (getAllCmps a s)) end)) | Error _ => false end) (sentIntents (state s))=false).
...
Proof.
    ...
    destruct H.
    destruct_conj H.
    assert ( (existsb (fun pair : iCmp * Intent => negb (isSomethingBool idCmp (cmpName (snd pair))) && (if idInt_eq (idI (snd pair)) (idI i) then true else false) && match map_apply iCmp_eq (running (state s)) (fst pair) with | Value c1 => existsb (fun iFil : intentFilter => actionTestBool (snd pair) iFil s && categoryTestBool (snd pair) iFil s && dataTestBool (snd pair) iFil s) (MyList.concat (map getFilters match intType (snd pair) with | intActivity => filter (fun c2 : Cmp => canStartBool c1 c2 s) (filter isActivityBool (getAllCmps a s)) | intService => filter (fun c2 : Cmp => canStartBool c1 c2 s) (filter isServiceBool (getAllCmps a s)) | intBroadcast => filter (fun c2 : Cmp => canStartBool c1 c2 s) (filter isBroadReceiverBool (getAllCmps a s)) end)) | Error _ => false end) (sentIntents (state s)))=true).
    rewrite existsb_exists.
    destruct H2.
    exists (x0,x).
    destruct H1.
