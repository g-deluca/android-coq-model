...
Proof.
    ...
    unfold removeAllPerms.
    remember (map (fun tuple : A * CProvider * uri => (tuple, match map_apply domeq mp tuple with | Value pt' => match ptminus pt' pt with | Some pt'' => pt'' | None => Both end | Error _ => Both end)) (filter (fun tuple : A * CProvider * uri => match map_apply domeq mp tuple with | Value pt' => isSomethingBool PType (ptminus pt' pt) | Error _ => false end) (filter (fun tuple : A * CProvider * uri => if CProvider_eq cp (snd (fst tuple)) then if uri_eq u (snd tuple) then true else false else false) (map_getKeys mp)))) as indexAndVals.
    case_eq (map_apply domeq mp (ic', cp, u));intros.
    case_eq (ptminus p pt);intros.
    rewrite valueDropThenValue.
    split.
...
Proof.
    ...
    assert ( (forall (ic' : iCmp) (cp' : CProvider) (u' : uri) (pt' : PType), map_apply deltpermsdomeq (removeAllPerms iCmp deltpermsdomeq (delTPerms (state s)) cp u pt) (ic', cp', u') = Value (iCmp * CProvider * uri) pt' -> exists pt'' : PType, map_apply deltpermsdomeq (delTPerms (state s)) (ic', cp', u') = Value (iCmp * CProvider * uri) pt'' /\ (pt'' = pt' \/ cp' = cp /\ u' = u /\ ptminus pt'' pt = Some pt')) /\ (forall (ic' : iCmp) (cp' : CProvider) (u' : uri) (pt' : PType), map_apply deltpermsdomeq (delTPerms (state s)) (ic', cp', u') = Value (iCmp * CProvider * uri) pt' -> ptminus pt' pt = None /\ cp' = cp /\ u' = u \/ (exists pt'' : PType, map_apply deltpermsdomeq (removeAllPerms iCmp deltpermsdomeq (delTPerms (state s)) cp u pt) (ic', cp', u') = Value (iCmp * CProvider * uri) pt'' /\ (pt'' = pt' \/ cp' = cp /\ u' = u /\ ptminus pt' pt = Some pt''))) /\ (forall ic' : iCmp, match map_apply deltpermsdomeq (delTPerms (state s)) (ic', cp, u) with | Value pt' => match ptminus pt' pt with | Some pt'' => map_apply deltpermsdomeq (removeAllPerms iCmp deltpermsdomeq (delTPerms (state s)) cp u pt) (ic', cp, u) = Value (iCmp * CProvider * uri) pt'' | None => ~ is_Value (map_apply deltpermsdomeq (removeAllPerms iCmp deltpermsdomeq (delTPerms (state s)) cp u pt) (ic', cp, u)) end | Error _ => ~ is_Value (map_apply deltpermsdomeq (removeAllPerms iCmp deltpermsdomeq (delTPerms (state s)) cp u pt) (ic', cp, u)) end)).
